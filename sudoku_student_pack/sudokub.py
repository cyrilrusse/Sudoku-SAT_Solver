...
#        output("c %s\n"%s)
    ...
    square_len = int(math.sqrt(N))+1
    for square_i in range(1, square_len):
        for square_j in range(1, square_len):
            for k in range(1, N+1):
                for i in range(square_i*square_len, (square_i*square_len)+square_len):
                    for j in range(square_j*square_len, (square_j*square_len)+square_len):
                        for i2 in range(i+1, (square_i*square_len)+square_len):
                            for j2 in range(j+1, (square_j*square_len)+square_len):
                                newneglit(i, j, k)
                                newneglit(i2, j2, k)
                                newcl()
